  ...
  return (
    <section className='max-md:w-full flex flex-col gap-4 w-96 mx-auto'>
      ...
      <form onSubmit={handleSubmit}>
        <section className={`grid grid-cols-1 ${fieldPerRow > 1 ? `md:grid-cols-${fieldPerRow}` : ''} mb-4 gap-4`}>
          {fields.map((field) => (
        ...
        >
          <button
            ...
            dark:text-white rounded-md h-10 font-medium shadow-[0px_1px_0px_0px_#ffffff40_inset,0px_-1px_0px_0px_#ffffff40_inset] 
              dark:shadow-[0px_1px_0px_0px_var(--zinc-800)_inset,0px_-1px_0px_0px_var(--zinc-800)_inset] outline-none hover:cursor-pointer disabled:opacity-50'
            type='submit'
            disabled={isLoading}
          >
            {isLoading ? (
